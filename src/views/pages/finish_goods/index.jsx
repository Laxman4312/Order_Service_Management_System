...
import { FinishGoodsIcon } from 'config/icons';
...
import FinishGoodsDialog from './Dialog';
...
const LocationMetaData = () => {
  ...
  const { getDialogState, handleDialogOpen, handleDialogClose } = useMultiDialogNavigation(
    {
      finish_goods_form_dialog: () => null,
    },
    // { persistData: true },
  );

  // Get individual dialog states
  const formDialog = getDialogState('finish_goods_form_dialog');
  ...
  React.useEffect(() => {
    (async () => {
      ...
      await fetchLocationMetaData(params);
    })();
  }, [page, rowsPerPage, restoreIsDeleted, isSaving, debouncedQuery, fetchLocationMetaData]);

  // Event handlers
  const handleEdit = (item) => {
    handleDialogOpen('finish_goods_form_dialog', 'Edit', item);
  };
  ...
  const pageConfig = {
    isLoading,
    isError,
    pageFavicon: FinishGoodsIcon,
    title: restoreIsDeleted ? 'Restore Finish Goods' : 'Finish Goods',
    pageButtons: [
      {
        buttonTitle: 'Add Finish Goods Products',
        startIcon: <IconShoppingCartPlus />,
        buttonName: 'Add',
        handleChange: () => handleDialogOpen('finish_goods_form_dialog'),
      },
      {
        buttonTitle: restoreIsDeleted ? 'Back' : 'Restore Finish Goods Products',
        buttonName: restoreIsDeleted ? 'Back' : 'Restore',
        startIcon: restoreIsDeleted ? <IconArrowBackUpDouble /> : <IconShoppingCartCog />,
    ...
    <>
      ...
      {formDialog.isOpen && (
        <FinishGoodsDialog
          ...
          handleCloseModal={() => handleDialogClose('finish_goods_form_dialog')}
          isSaving={isSaving}
