...
const FinishGoodsDialog = ({
  ...
  const LocationFormContent = ({ formikProps, buttonName }) => {
    useEffect(() => {
      let testData = '';
      if (typeof formikProps.values?.test === 'object') {
        testData = formikProps.values?.test?.color_code ?? '';
      } else {
        testData = formikProps.values?.test;
      }

      if (formikProps.values?.testOptions && testData) {
        const searchRes = formikProps.values?.testOptions.find(
          (item) => item.color_code.toLowerCase() === testData.toLowerCase(),
        );

        if (searchRes) {
          formikProps.setValues((prev) => ({
            ...prev,
            loc_name: searchRes.color_code,
          }));
        } else {
          formikProps.setValues((prev) => ({
            ...prev,
    ...
    }, [formikProps.values.test, formikProps.values.testOptions]);
  ...
  return (
    <ProjectDialog open={isOpen} maxWidth="md" handleClose={handleCloseModal}>
      ...
      >
        ...
        >
          {(formikProps) => (
            <LocationFormContent formikProps={formikProps} buttonName={buttonName} />
          )}
        </Form>
      </FormLayout>
    </ProjectDialog>
...
export default FinishGoodsDialog;
