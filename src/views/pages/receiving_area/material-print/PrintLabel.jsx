...
const RenderData = ({ printData }) => {
  const data = [
    { label: 'PO Number', labelData: `${printData?.po_number ?? ''}` },
    // { label: 'Location Code', labelData: `${printData?.loc_code ?? ''}` },
    // { label: 'Item Code', labelData: `${printData?.item_code ?? ''}` },
    {
      label: 'Item Name',
      labelData: `${printData?.particulars ?? ''}`,
    },
    ...
    <Grid container alignItems="center" justifyContent="space-between" sx={{ userSelect: 'none' }}>
      ...
      >
        <Box sx={{ mb: 0, display: 'flex', justifyContent: 'flex-start' }}>
          <Image image={BrandLogo} alt="JSW_Logo" width="90" />
        </Box>
        <Box width="100%">
          {data.map((item, index) => (
            <Box display="flex" columnGap={1} key={index}>
              <Box flexGrow={1} sx={{ width: '30%' }}>
              ...
              <Box flexGrow={1} sx={{ width: '65%' }}>
                ...
                >
                  {truncateText(item.labelData, 75)}
                </Typography>
              </Box>
            </Box>
          ))}
